...
##y_ref_trajectory
    ...
    y_poly = 0
    for i in range(rg + 1):
        y_poly = y_poly + h_r * sp.factorial(2 * rg + 1) / (sp.factorial(rg) * T_tra ** (2 * rg + 1)) * (
    ...
    for i in range(100):
        ...
        if T_act > T_tra_value:
            u_act = u_input_T.subs([(x1, xx_act[0]), (x2, xx_act[1]), (x3, xx_act[2])])
        if xx_act[0] <= 0:
            xx_act[0] = 0
            u_act=255
        if xx_act[0] <= 0 and xx_act[1] <= 0:
